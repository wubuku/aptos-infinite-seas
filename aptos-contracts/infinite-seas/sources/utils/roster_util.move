module infinite_seas::roster_util {
    use std::option;
    ...
    public fun add_ship_id(ship_ids: &mut vector<address>, ship_id: address, position: Option<u64>) {
        if (option::is_none(&position)) {
            ...
            if (idx >= vector::length(ship_ids)) {
                ...
                vector::insert(ship_ids, idx, ship_id);
            }
        };
    }
    ...
    public fun calculate_roster_speed(roster: &Roster): u32 {
        ...
        while (i < l) {
            ...
            assert!(object::object_exists<Ship>(ship_id), EInconsistentRosterShipId);
            //let ship = object_table::borrow(ships, ship_id);
            let shop_pass_obj = ship::get_ship(ship_id);
            let ship = pass_object::borrow(&shop_pass_obj);
            speed = speed + ship::speed(ship);
            ship::return_ship(shop_pass_obj);
            i = i + 1;
        };
        speed / (l as u32)
    }

    /// Returns true if all ships in the roster are destroyed except the ship with the given address.
    public fun is_destroyed_except_ship(roster: &Roster, ship: address): bool {
        let ship_ids = roster::borrow_ship_ids(roster);
        //let ships = roster::borrow_ships(roster);
        let i = 0;
        let l = vector::length(ship_ids);
