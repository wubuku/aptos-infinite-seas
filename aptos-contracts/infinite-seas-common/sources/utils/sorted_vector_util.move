module infinite_seas_common::sorted_vector_util {

    use std::bcs;
    use std::option::{Self, Option};
    use std::vector;

    use infinite_seas_common::compare;
    use infinite_seas_common::item_id_quantity_pair::{Self, ItemIdQuantityPair};
    ...
    public fun new_item_id_quantity_pairs(
        ...
        while (i < l) {
            ...
            insert_or_add_item_id_quantity_pair(
                &mut items,
                infinite_seas_common::item_id_quantity_pair::new(item_id, item_quantity)
            );
            i = i + 1;
        };
        items
    ...
    public fun item_id_quantity_pairs_multiply(
        ...
        while (i < l) {
            ...
            let item_id = infinite_seas_common::item_id_quantity_pair::item_id(&pair);
            let quantity = infinite_seas_common::item_id_quantity_pair::quantity(&pair);
            insert_or_add_item_id_quantity_pair(
                &mut items,
                infinite_seas_common::item_id_quantity_pair::new(item_id, quantity * multiplier)
            );
            i = i + 1;
        };
        items
    ...
    public fun add_id(v: &mut vector<address>, id: address) {
        let (idx, low) = binary_search_id(v, id);
        if (option::is_some(&idx)) {
            abort EItemAlreadyExists
        };
        vector::insert(v, low, id);
    }

    public fun remove_id(v: &mut vector<address>, id: address) {
    ...
    public fun find_id(v: &vector<address>, id: address): Option<u64> {
        let (idx, _low) = binary_search_id(v, id);
        idx
    }

    fun binary_search_id(v: &vector<address>, id: address): (Option<u64>, u64) {
        let low = 0;
        let high = vector::length(v);
        while (low < high) {
            let mid = low + (high - low) / 2;
            let mid_value = vector::borrow(v, mid);
            let c = compare::cmp_bcs_bytes(&bcs::to_bytes(mid_value), &bcs::to_bytes(&id));
            if (c == EQUAL) {
                return (option::some(mid), low)
            } else if (c == LESS_THAN) {
                low = mid + 1;
    ...
    public fun concat_ids_bytes(ids: &vector<address>): vector<u8> {
