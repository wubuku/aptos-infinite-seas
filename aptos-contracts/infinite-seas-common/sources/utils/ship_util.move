module infinite_seas_common::ship_util {
    ...
    public fun remove_ship_id(ship_ids: &mut vector<address>, ship_id: address) {
        let i = 0;
    ...
    public fun find_ship_id(ship_ids: &vector<address>, ship_id: address): Option<u64> {
        let i = 0;
        let l = vector::length(ship_ids);
        while (i < l) {
            if (*vector::borrow(ship_ids, i) == ship_id) {
